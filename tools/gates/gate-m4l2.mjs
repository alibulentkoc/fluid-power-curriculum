...
{
  const dom=new JSDOM(readFileSync(B+'/demos/lesson02_speed.html','utf8'),{runScripts:'dangerously',pretendToBeVisual:true});
  const d=dom.window.document,W=dom.window; const $=id=>d.getElementById(id);
  const setQ=v=>{$('flowRange').value=v;$('flowRange').dispatchEvent(new W.Event('input'));};
  const ve=()=>$('vextOut').textContent, vr=()=>$('vretOut').textContent, R=()=>$('ratioOut').textContent, P=()=>$('powerOut').textContent;
  setQ(10); ok('at 10 L/min: extend 85 mm/s, retract 124 mm/s', ve()==='85' && vr()==='124', ve()+'/'+vr());
  ok('speed ratio = area ratio 1.457, power 1.67 kW (both)', R()==='1.457' && P()==='1.67', R()+'/'+P());
  ok('extend/retract times ~7.1 s / 4.8 s over 600 mm', $('textOut').textContent==='7.1' && $('tretOut').textContent==='4.8', $('textOut').textContent+'/'+$('tretOut').textContent);
  setQ(6);  ok('at 6 L/min: extend 51, retract 74 mm/s (scales with flow)', ve()==='51' && vr()==='74', ve()+'/'+vr());
  ok('demo controls + readouts present', ['viz','barExt','barRet','flowRange','flowOut','vextOut','vretOut','textOut','tretOut','ratioOut','powerOut'].every(id=>$(id)));
}
// QUIZ
{
...
// LESSON
{
  const L=readFileSync(B+'/02_speed_from_flow.md','utf8');
  ...
  ok('DoD6/7 references reviewed SVG in <figure markdown>', L.includes('assets/m04-l2-speed.svg') && /<figure markdown>/.test(L));
  ok('DoD8 demo embedded (iframe) + open-in-tab link', /<iframe[^>]*demos\/lesson02_speed\.html/.test(L) && /\]\(demos\/lesson02_speed\.html\)/.test(L));
  ...
  ok('DoD11 advances narrative (Lesson 01 in, friction/Lesson 03 out)', /Lesson 01|Module 04/.test(L) && /Lesson 03|[Ff]riction/.test(L));
  ok('speed decision present (85/124 mm/s + ratio 1.457 + power conserved)', /85\s*mm|124\s*mm|Q\s*\/\s*A/i.test(L) && /1\.457/.test(L));
...
console.log(fail?('\n[gate-m4l2] FAIL '+fail):'\n[gate-m4l2] PASS  M04 L02 meets all Definition-of-Done checks + power demo/quiz + crisp vector preview');
