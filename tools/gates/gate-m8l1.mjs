...
const B='/mnt/user-data/outputs/module08_command';
let fail=0; const ok=(n,c,d='')=>{ if(!c){fail++;console.log('  FAIL '+n+(d?'  '+d:''))}else console.log('  ok   '+n); };
// DEMO: hydraulic power P = p*Q
{
  const dom=new JSDOM(readFileSync(B+'/demos/lesson01_command.html','utf8'),{runScripts:'dangerously',pretendToBeVisual:true});
  const d=dom.window.document,W=dom.window; const $=id=>d.getElementById(id);
  const set=v=>{$('u').value=v;$('u').dispatchEvent(new W.Event('input'));};
  set(100); ok('100% command -> ~85 mm/s, spool fully shifted', /85\.0 mm\/s/.test($('rv').textContent) && $('spool').getAttribute('transform')==='translate(46,0)');
  set(5);   ok('5% command -> ~4.2 mm/s creep', /4\.[0-9] mm\/s/.test($('rv').textContent) && /creep|position/i.test($('note').textContent));
  set(50);  ok('50% command -> ~42 mm/s, 5 L/min', /42\.[0-9] mm\/s/.test($('rv').textContent) && /5\.0 L\/min/.test($('rq').textContent));
  ok('command chain widgets present', ['u','rv','rq','ropen','spool','vbar','gpt','note'].every(id=>$(id)));
}
// QUIZ
{
...
// LESSON
{
  const L=readFileSync(B+'/01_commanding_with_electricity.md','utf8');
  ...
  ok('DoD6/7 references reviewed SVG in <figure markdown>', L.includes('assets/m08-l1-command.svg') && /<figure markdown>/.test(L));
  ok('DoD8 demo embedded (iframe) + open-in-tab link', /<iframe[^>]*demos\/lesson01_command\.html/.test(L) && /\]\(demos\/lesson01_command\.html\)/.test(L));
  ...
  ok('DoD11 opens module (lever/manual in, Lesson 02/03 sensor out)', /lever|manual/i.test(L) && /Lesson 0[23]|sensor/i.test(L));
  ok('command decision present (current->spool->flow->speed)', /command/i.test(L) && /spool|solenoid/i.test(L) && /speed|flow/i.test(L));
...
console.log(fail?('\n[gate-m8l1] FAIL '+fail):'\n[gate-m8l1] PASS  M08 L01 meets all Definition-of-Done checks + power demo/quiz + crisp vector preview');
