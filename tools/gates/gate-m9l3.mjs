...
const B='/mnt/user-data/outputs/module09_numerical';
let fail=0; const ok=(n,c,d='')=>{ if(!c){fail++;console.log('  FAIL '+n+(d?'  '+d:''))}else console.log('  ok   '+n); };
// DEMO: hydraulic power P = p*Q
{
  const dom=new JSDOM(readFileSync(B+'/demos/lesson03_numerical.html','utf8'),{runScripts:'dangerously',pretendToBeVisual:true});
  const d=dom.window.document,W=dom.window; const $=id=>d.getElementById(id);
  const set=v=>{$('dt').value=v;$('dt').dispatchEvent(new W.Event('input'));};
  set(1);  ok('1 ms -> stable+accurate, ~82 samples/osc', /stable\+accurate/.test($('rstat').textContent) && /8[0-9]/.test($('rspp').textContent) && $('verdict').className.includes('ok'));
  set(10); ok('10 ms -> stable but coarse', /coarse/.test($('rstat').textContent) && $('verdict').className.includes('edge'));
  set(30); ok('30 ms -> diverges (past 26 ms limit)', /DIVERGED/.test($('rstat').textContent) && $('verdict').className.includes('bad'));
  ok('time-step + curve widgets present', ['dt','curve','rspp','rpeak','rstat','verdict','tgt'].every(id=>$(id)) && $('curve').getAttribute('points').length>0);
}
// QUIZ
{
...
// LESSON
{
  const L=readFileSync(B+'/03_solving_it_numerically.md','utf8');
  ...
  ok('DoD6/7 references reviewed SVG in <figure markdown>', L.includes('assets/m09-l3-numerical.svg') && /<figure markdown>/.test(L));
  ok('DoD8 demo embedded (iframe) + open-in-tab link', /<iframe[^>]*demos\/lesson03_numerical\.html/.test(L) && /\]\(demos\/lesson03_numerical\.html\)/.test(L));
  ...
  ok('DoD11 advances (Lesson 02 in, Lesson 04/validation out)', /Lesson 02|equation/i.test(L) && /Lesson 04|validat/i.test(L));
  ok('numerical decision present (integration, time step, stability)', /integrat|Euler|step/i.test(L) && /stabilit|diverge|accura/i.test(L) && /26 ms|2\/|1 ms|dt/i.test(L));
...
console.log(fail?('\n[gate-m9l3] FAIL '+fail):'\n[gate-m9l3] PASS  M09 L03 meets all Definition-of-Done checks + power demo/quiz + crisp vector preview');
