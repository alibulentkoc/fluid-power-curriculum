...
const B='/mnt/user-data/outputs/module07_states';
let fail=0; const ok=(n,c,d='')=>{ if(!c){fail++;console.log('  FAIL '+n+(d?'  '+d:''))}else console.log('  ok   '+n); };
// DEMO: hydraulic power P = p*Q
{
  const dom=new JSDOM(readFileSync(B+'/demos/lesson03_states.html','utf8'),{runScripts:'dangerously',pretendToBeVisual:true});
  ...
  pk('raise'); ok('raise lights cap, pilot idle, ~100 bar', $('grp-cap').style.opacity==='1' && $('grp-pilot').style.opacity==='0.1' && /100 bar/.test($('note').textContent));
  pk('hold');  ok('hold: cap idle, pump loop, drop-tight', $('grp-cap').style.opacity==='0.1' && $('grp-supply').style.opacity==='1' && /drop-tight/.test($('note').textContent));
  pk('lower'); ok('lower: pilot active, metered descent', $('grp-pilot').style.opacity==='1' && /metered/.test($('note').textContent));
  ok('circuit groups + state selectors present', ['c','note','grp-supply','grp-return','grp-cap','grp-rod','grp-pilot'].every(id=>$(id)));
}
// QUIZ
{
...
// LESSON
{
  const L=readFileSync(B+'/03_tracing_the_operating_states.md','utf8');
  ...
  ok('DoD6/7 references reviewed SVG in <figure markdown>', L.includes('assets/m07-l3-states.svg') && /<figure markdown>/.test(L));
  ok('DoD8 demo embedded (iframe) + open-in-tab link', /<iframe[^>]*demos\/lesson03_states\.html/.test(L) && /\]\(demos\/lesson03_states\.html\)/.test(L));
  ...
  ok('DoD11 advances narrative (Lesson 02 in, Lesson 04/budget out)', /Lesson 02|assembled/.test(L) && /Lesson 04|budget/.test(L));
  ok('states decision present (raise/hold/lower paths)', /raise/i.test(L) && /hold/i.test(L) && /lower/i.test(L) && /path|trace|position/i.test(L));
...
console.log(fail?('\n[gate-m7l3] FAIL '+fail):'\n[gate-m7l3] PASS  M07 L03 meets all Definition-of-Done checks + power demo/quiz + crisp vector preview');
