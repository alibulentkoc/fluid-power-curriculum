...
{
  const dom=new JSDOM(readFileSync(B+'/demos/lesson03_bulk_modulus.html','utf8'),{runScripts:'dangerously',pretendToBeVisual:true});
  const d=dom.window.document,W=dom.window; const $=id=>d.getElementById(id);
  const setA=v=>{$('airRange').value=v;$('airRange').dispatchEvent(new W.Event('input'));};
  const be=()=>$('beOut').textContent, sag=()=>$('sagOut').textContent, vd=()=>$('verdict').textContent;
  setA(0);   ok('clean oil: Be=1.80 GPa, sag 0.34 mm, holds', be()==='1.80' && sag()==='0.34' && !/past/.test(vd()), be()+'/'+sag());
  setA(0.5); ok('0.5% air: softens (Be~0.95) but still holds', be()==='0.95' && !/past/.test(vd()), be()+'/'+sag());
  setA(1.5); ok('1.5% air: Be~0.49, sag>1 mm, FAILS ±1 mm', be()==='0.49' && parseFloat(sag())>1 && /past/.test(vd()), be()+'/'+sag());
  ok('sag bar + Be bar + threshold + readouts present', ['viz','sagBar','beBar','thr','airRange','beOut','kOut','sagOut','verdict'].every(id=>$(id)));
}
// QUIZ
{
...
// LESSON
{
  const L=readFileSync(B+'/03_bulk_modulus.md','utf8');
  ...
  ok('DoD6/7 references reviewed SVG in <figure markdown>', L.includes('assets/m03-l3-bulk-modulus.svg') && /<figure markdown>/.test(L));
  ok('DoD8 demo embedded (iframe) + open-in-tab link', /<iframe[^>]*demos\/lesson03_bulk_modulus\.html/.test(L) && /\]\(demos\/lesson03_bulk_modulus\.html\)/.test(L));
  ...
  ok('DoD11 advances narrative (Lesson 02 in, cleanliness/Lesson 04 out)', /Lesson 02|Module 03/.test(L) && /Lesson 04|[Cc]leanliness/.test(L));
  ok('stiffness decision present (bulk modulus 1.8 GPa + keep air out)', /1\.8\s*GPa|bulk modulus/i.test(L) && /air/i.test(L));
...
console.log(fail?('\n[gate-m3l3] FAIL '+fail):'\n[gate-m3l3] PASS  M03 L03 meets all Definition-of-Done checks + power demo/quiz + crisp vector preview');
