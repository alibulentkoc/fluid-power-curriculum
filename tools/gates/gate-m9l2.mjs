...
const B='/mnt/user-data/outputs/module09_eom';
let fail=0; const ok=(n,c,d='')=>{ if(!c){fail++;console.log('  FAIL '+n+(d?'  '+d:''))}else console.log('  ok   '+n); };
// DEMO: hydraulic power P = p*Q
{
  const dom=new JSDOM(readFileSync(B+'/demos/lesson02_eom.html','utf8'),{runScripts:'dangerously',pretendToBeVisual:true});
  ...
  set('p',100); set('v',0); ok('100 bar, v=0 -> net ~0, balanced (holds)', /balanced/.test($('rstate').textContent) && Math.abs(parseFloat($('rnet').textContent))<60);
  set('p',110); ok('110 bar -> positive acceleration (rises)', /accel/.test($('rstate').textContent) && parseFloat($('ra').textContent)>0.5);
  set('x',300); ok('mid-stroke -> ~589 cm3, f_n ~12 Hz', /589 cm/.test($('rV').textContent) && /12\.[0-9] Hz/.test($('rfn').textContent));
  set('x',600); ok('top of stroke -> lower f_n (softer spring)', parseFloat($('rfn').textContent)<12);
  ok('force-balance + oil-spring widgets present', ['p','v','x','rnet','ra','rstate','rV','rk','rfn','note'].every(id=>$(id)));
}
// QUIZ
{
...
// LESSON
{
  const L=readFileSync(B+'/02_the_equations_of_motion.md','utf8');
  ...
  ok('DoD6/7 references reviewed SVG in <figure markdown>', L.includes('assets/m09-l2-eom.svg') && /<figure markdown>/.test(L));
  ok('DoD8 demo embedded (iframe) + open-in-tab link', /<iframe[^>]*demos\/lesson02_eom\.html/.test(L) && /\]\(demos\/lesson02_eom\.html\)/.test(L));
  ...
  ok('DoD11 advances (Lesson 01 in, Lesson 03/numerical out)', /Lesson 01|kinematic|model/i.test(L) && /Lesson 03|numeric|solv|integrat/i.test(L));
  ok('equations decision present (three coupled equations, resonance)', /Newton|equation/i.test(L) && /compressib|pressure|chamber/i.test(L) && /natural frequency|12 Hz|spring/i.test(L));
...
console.log(fail?('\n[gate-m9l2] FAIL '+fail):'\n[gate-m9l2] PASS  M09 L02 meets all Definition-of-Done checks + power demo/quiz + crisp vector preview');
