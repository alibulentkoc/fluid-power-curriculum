...
const B='/mnt/user-data/outputs/module06_holding';
let fail=0; const ok=(n,c,d='')=>{ if(!c){fail++;console.log('  FAIL '+n+(d?'  '+d:''))}else console.log('  ok   '+n); };
// DEMO: hydraulic power P = p*Q
{
  const dom=new JSDOM(readFileSync(B+'/demos/lesson03_holding.html','utf8'),{runScripts:'dangerously',pretendToBeVisual:true});
  const d=dom.window.document,W=dom.window; const $=id=>d.getElementById(id);
  const set=v=>{$('leakRange').value=v;$('leakRange').dispatchEvent(new W.Event('input'));};
  const D=()=>$('driftOut').textContent, T=()=>$('timeOut').textContent, cls=()=>$('note').className;
  set(10); ok('spool 10 cc/min: drifts 0.085 mm/s, breaches +/-1 mm in ~12 s (fails)', D()==='0.085' && /11\.8/.test(T()) && /bad/.test(cls()), D()+'/'+T());
  set(20); ok('spool 20 cc/min: drift doubles to 0.170 mm/s, ~6 s', D()==='0.170', D());
  $('bPOCV').dispatchEvent(new W.Event('click')); ok('POCV: drop-tight, 0 drift, never breaches (holds)', D()==='0.000' && /never/.test(T()) && /ok/.test(cls()), D()+'/'+T());
  ok('drift trace + hold-type buttons + leakage slider + readouts present', ['chart','trace','bSpool','bPOCV','leakRange','driftOut','timeOut'].every(id=>$(id)));
}
// QUIZ
{
...
// LESSON
{
  const L=readFileSync(B+'/03_holding_the_load.md','utf8');
  ...
  ok('DoD6/7 references reviewed SVG in <figure markdown>', L.includes('assets/m06-l3-holding.svg') && /<figure markdown>/.test(L));
  ok('DoD8 demo embedded (iframe) + open-in-tab link', /<iframe[^>]*demos\/lesson03_holding\.html/.test(L) && /\]\(demos\/lesson03_holding\.html\)/.test(L));
  ...
  ok('DoD11 advances narrative (Lesson 02 in, metering/Lesson 04 out)', /Lesson 02|Module 06/.test(L) && /Lesson 04|meter|proportional/.test(L));
  ok('load-holding decision present (POCV + drop-tight)', /pilot.operated check|POCV|leak.free|drop.tight/i.test(L) && /drift|hold|33 ?bar/i.test(L));
...
console.log(fail?('\n[gate-m6l3] FAIL '+fail):'\n[gate-m6l3] PASS  M06 L03 meets all Definition-of-Done checks + power demo/quiz + crisp vector preview');
