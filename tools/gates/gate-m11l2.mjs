...
const B='/mnt/user-data/outputs/module11_residual';
let fail=0; const ok=(n,c,d='')=>{ if(!c){fail++;console.log('  FAIL '+n+(d?'  '+d:''))}else console.log('  ok   '+n); };
// DEMO: hydraulic power P = p*Q
{
  const dom=new JSDOM(readFileSync(B+'/demos/lesson02_residual.html','utf8'),{runScripts:'dangerously',pretendToBeVisual:true});
  const d=dom.window.document,W=dom.window; const $=id=>d.getElementById(id);
  const set=(k,w)=>{$('k').value=k;$('w').value=w;$('k').dispatchEvent(new W.Event('input'));};
  set(1.6,1);  ok('tight threshold -> many false alarms', parseInt($('rfa').textContent)>3 && $('verdict').className.includes('warn'));
  set(3.0,20); ok('balanced (3 sigma + filter) -> few false alarms, drift caught', parseInt($('rfa').textContent)<=2 && $('rdet').textContent==='yes' && $('verdict').className.includes('ok'));
  set(4.5,3);  ok('loose threshold -> no false alarms', parseInt($('rfa').textContent)===0);
  ok('residual widgets present', ['k','w','rthr','rfa','rdet','verdict','thr','raw','filt'].every(id=>$(id)) && $('filt').getAttribute('points').length>0);
}
// QUIZ
{
...
// LESSON
{
  const L=readFileSync(B+'/02_the_residual_signal.md','utf8');
  ...
  ok('DoD6/7 references reviewed SVG in <figure markdown>', L.includes('assets/m11-l2-residual.svg') && /<figure markdown>/.test(L));
  ok('DoD8 demo embedded (iframe) + open-in-tab link', /<iframe[^>]*demos\/lesson02_residual\.html/.test(L) && /\]\(demos\/lesson02_residual\.html\)/.test(L));
  ...
  ok('DoD11 advances (Lesson 01 in, Lesson 03/diagnose out)', /Lesson 01|live.*model|residual/i.test(L) && /Lesson 03|diagnos|tell.*apart|detect/i.test(L));
  ok('residual decision present (band, threshold, filter)', /residual|healthy band/i.test(L) && /threshold|sigma|\u03c3/i.test(L) && /noise|model error|filter|false alarm/i.test(L));
...
console.log(fail?('\n[gate-m11l2] FAIL '+fail):'\n[gate-m11l2] PASS  M11 L02 meets all Definition-of-Done checks + power demo/quiz + crisp vector preview');
