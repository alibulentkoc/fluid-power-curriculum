...
{
  const dom=new JSDOM(readFileSync(B+'/demos/lesson05_dynamics.html','utf8'),{runScripts:'dangerously',pretendToBeVisual:true});
  const d=dom.window.document,W=dom.window; const $=id=>d.getElementById(id);
  const setBe=v=>{$('beRange').value=v;$('beRange').dispatchEvent(new W.Event('input'));};
  const fnv=()=>$('fnOut').textContent, k=()=>$('kOut').textContent;
  setBe(1.5); ok('loaded, Be=1.5 GPa: k~23 MN/m, f_n ~17 Hz', k()==='23' && fnv()==='17.1', k()+'/'+fnv());
  setBe(0.5); ok('air softens (Be=0.5): f_n falls to ~10 Hz', fnv()==='9.9', fnv());
  $('bTool').dispatchEvent(new W.Event('click')); setBe(1.5);
  ok('bare 3 kg tool: f_n much higher (~450 Hz)', fnv()==='442', fnv());
  ok('step-response chart + mass toggle + readouts present', ['chart','curve','target','band','beRange','kOut','fnOut','tsOut','bTool','bLoaded','note'].every(id=>$(id)));
}
// QUIZ
{
...
// LESSON
{
  const L=readFileSync(B+'/05_the_cylinder_in_motion.md','utf8');
  ...
  ok('DoD6/7 references reviewed SVG in <figure markdown>', L.includes('assets/m04-l5-dynamics.svg') && /<figure markdown>/.test(L));
  ok('DoD8 demo embedded (iframe) + open-in-tab link', /<iframe[^>]*demos\/lesson05_dynamics\.html/.test(L) && /\]\(demos\/lesson05_dynamics\.html\)/.test(L));
  ...
  ok('DoD11 advances narrative (Module 04 closes, hands to Module 05)', /Lesson 04|Module 04/.test(L) && /Module 05/.test(L));
  ok('dynamics decision present (mass-spring + natural frequency)', /natural frequency|mass.{0,6}spring|f_n|f\u2099/i.test(L) && /17\s*Hz|B_e|B\u2091|stiff/i.test(L));
...
console.log(fail?('\n[gate-m4l5] FAIL '+fail):'\n[gate-m4l5] PASS  M04 L05 meets all Definition-of-Done checks + power demo/quiz + crisp vector preview');
