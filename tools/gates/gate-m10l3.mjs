...
const B='/mnt/user-data/outputs/module10_pid';
let fail=0; const ok=(n,c,d='')=>{ if(!c){fail++;console.log('  FAIL '+n+(d?'  '+d:''))}else console.log('  ok   '+n); };
// DEMO: hydraulic power P = p*Q
{
  const dom=new JSDOM(readFileSync(B+'/demos/lesson03_pid.html','utf8'),{runScripts:'dangerously',pretendToBeVisual:true});
  const d=dom.window.document,W=dom.window; const $=id=>d.getElementById(id);
  $('bP').dispatchEvent(new W.Event('click'));   ok('P only -> residual droop ~0.24 mm', /0\.2[0-9] mm/.test($('rerr').textContent) && $('verdict').className.includes('warn'));
  $('bPI').dispatchEvent(new W.Event('click'));  ok('PI -> integral erases droop (~0 mm)', /\+?0\.00 mm/.test($('rerr').textContent) && $('verdict').className.includes('ok'));
  $('bPID').dispatchEvent(new W.Event('click')); ok('PID -> zero error, damped', /0\.00 mm/.test($('rerr').textContent) && $('verdict').className.includes('ok'));
  ok('PID widgets present', ['bP','bPI','bPID','curve','rerr','ros','rterms','verdict','tgt'].every(id=>$(id)) && $('curve').getAttribute('points').length>0);
}
// QUIZ
{
...
// LESSON
{
  const L=readFileSync(B+'/03_pid_control.md','utf8');
  ...
  ok('DoD6/7 references reviewed SVG in <figure markdown>', L.includes('assets/m10-l3-pid.svg') && /<figure markdown>/.test(L));
  ok('DoD8 demo embedded (iframe) + open-in-tab link', /<iframe[^>]*demos\/lesson03_pid\.html/.test(L) && /\]\(demos\/lesson03_pid\.html\)/.test(L));
  ...
  ok('DoD11 advances (Lesson 02 in, Lesson 04/tuning out)', /Lesson 02|proportional/i.test(L) && /Lesson 04|tuning|stability/i.test(L));
  ok('PID decision present (integral erases droop, derivative damps)', /integral|derivative|PID/i.test(L) && /droop|damp/i.test(L) && /erase|zero|exact|ring/i.test(L));
...
console.log(fail?('\n[gate-m10l3] FAIL '+fail):'\n[gate-m10l3] PASS  M10 L03 meets all Definition-of-Done checks + power demo/quiz + crisp vector preview');
