...
const B='/mnt/user-data/outputs/module10_closed_loop';
let fail=0; const ok=(n,c,d='')=>{ if(!c){fail++;console.log('  FAIL '+n+(d?'  '+d:''))}else console.log('  ok   '+n); };
// DEMO: hydraulic power P = p*Q
{
  const dom=new JSDOM(readFileSync(B+'/demos/lesson05_closed_loop.html','utf8'),{runScripts:'dangerously',pretendToBeVisual:true});
  const d=dom.window.document,W=dom.window; const $=id=>d.getElementById(id);
  ok('500 mm mission reaches ~6 s and holds within 1 mm', /6\.[0-9] s/.test($('dt').textContent) && parseFloat($('dh').textContent)<1);
  $('dist').dispatchEvent(new W.Event('click'));
  ok('rejects load disturbance to within 1 mm', parseFloat($('dr').textContent)<1);
  const seth=v=>{$('h').value=v;$('h').dispatchEvent(new W.Event('input'));};
  seth(300); ok('shorter target reaches sooner, still holds & recovers', parseFloat($('dt').textContent)<5 && parseFloat($('dr').textContent)<1);
  ok('closed-loop widgets present', ['h','nodist','dist','dt','dh','dr','note','curve','band','tgt'].every(id=>$(id)) && $('curve').getAttribute('points').length>0);
}
// QUIZ
{
...
// LESSON
{
  const L=readFileSync(B+'/05_the_closed_loop_platform.md','utf8');
  ...
  ok('DoD6/7 references reviewed SVG in <figure markdown>', L.includes('assets/m10-l5-closed-loop.svg') && /<figure markdown>/.test(L));
  ok('DoD8 demo embedded (iframe) + open-in-tab link', /<iframe[^>]*demos\/lesson05_closed_loop\.html/.test(L) && /\]\(demos\/lesson05_closed_loop\.html\)/.test(L));
  ...
  ok('DoD11 closes module (Lesson 04 in, Module 11 out)', /Lesson 04|tuning|margin/i.test(L) && /Module 11|digital model|closes/i.test(L));
  ok('closed-loop decision present (meets +/-1 mm, rejects disturbance)', /closed-loop|mission|feedforward|profile/i.test(L) && /\u00b11 mm|hold|reject/i.test(L) && /disturbance|500 mm|held/i.test(L));
...
console.log(fail?('\n[gate-m10l5] FAIL '+fail):'\n[gate-m10l5] PASS  M10 L05 meets all Definition-of-Done checks + power demo/quiz + crisp vector preview');
