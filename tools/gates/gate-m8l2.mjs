...
const B='/mnt/user-data/outputs/module08_valves';
let fail=0; const ok=(n,c,d='')=>{ if(!c){fail++;console.log('  FAIL '+n+(d?'  '+d:''))}else console.log('  ok   '+n); };
// DEMO: hydraulic power P = p*Q
{
  const dom=new JSDOM(readFileSync(B+'/demos/lesson02_valves.html','utf8'),{runScripts:'dangerously',pretendToBeVisual:true});
  const d=dom.window.document,W=dom.window; const $=id=>d.getElementById(id);
  const preset=id=>{$(id).dispatchEvent(new W.Event('click'));};
  preset('b-onoff'); ok('on/off preset -> 1.70 mm overshoot, 143 bar, verdict fails', /1\.70 mm/.test($('rover').textContent) && /143 bar/.test($('rp').textContent) && $('verdict').className.includes('bad'));
  preset('b-prop');  ok('proportional preset -> ~0.08 mm, ~102 bar, verdict ok', /0\.08 mm/.test($('rover').textContent) && /10[0-9] bar/.test($('rp').textContent) && $('verdict').className.includes('ok'));
  const set=v=>{$('v').value=v;$('v').dispatchEvent(new W.Event('input'));};
  set(85); ok('slider at 85 mm/s exceeds relief', $('verdict').className.includes('bad'));
  ok('valve-choice widgets present', ['v','rest','pbar','plbl','verdict','band','prelief','rover','rp'].every(id=>$(id)));
}
// QUIZ
{
...
// LESSON
{
  const L=readFileSync(B+'/02_on_off_vs_proportional.md','utf8');
  ...
  ok('DoD6/7 references reviewed SVG in <figure markdown>', L.includes('assets/m08-l2-valves.svg') && /<figure markdown>/.test(L));
  ok('DoD8 demo embedded (iframe) + open-in-tab link', /<iframe[^>]*demos\/lesson02_valves\.html/.test(L) && /\]\(demos\/lesson02_valves\.html\)/.test(L));
  ...
  ok('DoD11 advances (Lesson 01 in, Lesson 03/sensing out)', /Lesson 01|command/i.test(L) && /Lesson 03|sensing|position/i.test(L));
  ok('valve-choice decision present (on/off vs proportional, positioning)', /on.?off/i.test(L) && /proportional/i.test(L) && /position|overshoot|1 mm/i.test(L));
...
console.log(fail?('\n[gate-m8l2] FAIL '+fail):'\n[gate-m8l2] PASS  M08 L02 meets all Definition-of-Done checks + power demo/quiz + crisp vector preview');
