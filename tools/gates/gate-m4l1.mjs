...
{
  const dom=new JSDOM(readFileSync(B+'/demos/lesson01_force.html','utf8'),{runScripts:'dangerously',pretendToBeVisual:true});
  const d=dom.window.document,W=dom.window; const $=id=>d.getElementById(id);
  const setP=v=>{$('pRange').value=v;$('pRange').dispatchEvent(new W.Event('input'));};
  const F=()=>$('forceOut').textContent, Aa=()=>$('areaOut').textContent, R=()=>$('ratioOut').textContent;
  ok('extend @100 bar = 19.6 kN on full bore 1963.5 mm^2', F()==='19.6' && Aa()==='1963.5' && R()==='1.457', F()+'/'+Aa()+'/'+R());
  $('bRetract').dispatchEvent(new W.Event('click'));
  ok('retract @100 bar = 13.5 kN on annulus 1347.7 mm^2', F()==='13.5' && Aa()==='1347.7', F()+'/'+Aa());
  $('bExtend').dispatchEvent(new W.Event('click')); setP(150);
  ok('extend @150 bar = 29.5 kN (force scales with pressure)', F()==='29.5', F());
  ok('area ratio holds at 1.457 regardless of pressure', R()==='1.457', R());
  ok('demo controls + readouts present', ['viz','bore','rodHole','pRange','pOut','bExtend','bRetract','areaOut','forceOut','ratioOut','barExt','barRet','note'].every(id=>$(id)));
}
// QUIZ
{
...
// LESSON
{
  const L=readFileSync(B+'/01_force_from_pressure.md','utf8');
  ...
  ok('DoD6/7 references reviewed SVG in <figure markdown>', L.includes('assets/m04-l1-force.svg') && /<figure markdown>/.test(L));
  ok('DoD8 demo embedded (iframe) + open-in-tab link', /<iframe[^>]*demos\/lesson01_force\.html/.test(L) && /\]\(demos\/lesson01_force\.html\)/.test(L));
  ...
  ok('DoD11 advances narrative (Module 03 in, speed/Lesson 02 out)', /Module 0[23]/.test(L) && /Lesson 02|[Ss]peed/.test(L));
  ok('force decision present (19.6 kN extend + area ratio 1.457)', /19\.6\s*kN/i.test(L) && /1\.457|area ratio/i.test(L));
...
console.log(fail?('\n[gate-m4l1] FAIL '+fail):'\n[gate-m4l1] PASS  M04 L01 meets all Definition-of-Done checks + power demo/quiz + crisp vector preview');
