...
const B='/mnt/user-data/outputs/module11_observer';
let fail=0; const ok=(n,c,d='')=>{ if(!c){fail++;console.log('  FAIL '+n+(d?'  '+d:''))}else console.log('  ok   '+n); };
// DEMO: hydraulic power P = p*Q
{
  const dom=new JSDOM(readFileSync(B+'/demos/lesson04_observer.html','utf8'),{runScripts:'dangerously',pretendToBeVisual:true});
  const d=dom.window.document,W=dom.window; const $=id=>d.getElementById(id);
  const setl=v=>{$('l').value=v;$('l').dispatchEvent(new W.Event('input'));};
  setl(2);  const cl=parseFloat($('rconv').textContent);
  setl(35); const ch=parseFloat($('rconv').textContent); ok('higher gain converges faster', ch<cl && ch<1);
  setl(8);  ok('balanced gain -> converges ~1 s, clean velocity vs differentiation', /1\.[0-9]/.test($('rconv').textContent) && /28/.test($('rvel').textContent) && $('verdict').className.includes('ok'));
  ok('observer widgets present', ['l','curve','rconv','rnoise','rvel','verdict','zero'].every(id=>$(id)) && $('curve').getAttribute('points').length>0);
}
// QUIZ
{
...
// LESSON
{
  const L=readFileSync(B+'/04_estimating_hidden_state.md','utf8');
  ...
  ok('DoD6/7 references reviewed SVG in <figure markdown>', L.includes('assets/m11-l4-observer.svg') && /<figure markdown>/.test(L));
  ok('DoD8 demo embedded (iframe) + open-in-tab link', /<iframe[^>]*demos\/lesson04_observer\.html/.test(L) && /\]\(demos\/lesson04_observer\.html\)/.test(L));
  ...
  ok('DoD11 advances (Lesson 03 in, Lesson 05/monitored platform out)', /Lesson 03|diagnos|fault/i.test(L) && /Lesson 05|monitored platform|complete/i.test(L));
  ok('estimation decision present (observer, hidden state, gain)', /observer|estimat/i.test(L) && /hidden|velocity|unmeasured|state/i.test(L) && /gain|Kalman|model.*measurement|track/i.test(L));
...
console.log(fail?('\n[gate-m11l4] FAIL '+fail):'\n[gate-m11l4] PASS  M11 L04 meets all Definition-of-Done checks + power demo/quiz + crisp vector preview');
