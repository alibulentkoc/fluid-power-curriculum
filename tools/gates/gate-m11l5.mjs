...
const B='/mnt/user-data/outputs/module11_monitored';
let fail=0; const ok=(n,c,d='')=>{ if(!c){fail++;console.log('  FAIL '+n+(d?'  '+d:''))}else console.log('  ok   '+n); };
// DEMO: hydraulic power P = p*Q
{
  const dom=new JSDOM(readFileSync(B+'/demos/lesson05_monitored.html','utf8'),{runScripts:'dangerously',pretendToBeVisual:true});
  const d=dom.window.document,W=dom.window; const $=id=>d.getElementById(id);
  ok('detected early, fails much later -> large lead time', parseFloat($('rdet').textContent)<3 && parseFloat($('rfail').textContent)>10 && parseFloat($('rlead').textContent)>10);
  const seta=v=>{$('acc').value=v;$('acc').dispatchEvent(new W.Event('input'));};
  seta(4); ok('faster leak still detected early with lead time', parseFloat($('rdet').textContent)<2 && parseFloat($('rlead').textContent)>5);
  ok('verdict frames the lead-time / predictive-maintenance payoff', /lead time/i.test($('verdict').textContent) && /planned|repair/i.test($('verdict').textContent));
  ok('monitored widgets present', ['acc','pos','leak','rdet','rfail','rlead','verdict','detl','faill'].every(id=>$(id)) && $('pos').getAttribute('points').length>0);
}
// QUIZ
{
...
// LESSON
{
  const L=readFileSync(B+'/05_the_monitored_platform.md','utf8');
  ...
  ok('DoD6/7 references reviewed SVG in <figure markdown>', L.includes('assets/m11-l5-monitored.svg') && /<figure markdown>/.test(L));
  ok('DoD8 demo embedded (iframe) + open-in-tab link', /<iframe[^>]*demos\/lesson05_monitored\.html/.test(L) && /\]\(demos\/lesson05_monitored\.html\)/.test(L));
  ...
  ok('DoD11 closes module & curriculum (Lesson 04 in, complete out)', /Lesson 04|estimat|observer/i.test(L) && /complete|monitored|curriculum|closes/i.test(L));
  ok('monitored-platform decision present (detect/diagnose/quantify before failure)', /monitored|complete machine|assembl/i.test(L) && /detect|diagnos|quantif|lead time/i.test(L) && /before.*fail|predictive|behaving as designed/i.test(L));
...
console.log(fail?('\n[gate-m11l5] FAIL '+fail):'\n[gate-m11l5] PASS  M11 L05 meets all Definition-of-Done checks + power demo/quiz + crisp vector preview');
