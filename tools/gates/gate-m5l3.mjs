...
const B='/mnt/user-data/outputs/module05_motor';
let fail=0; const ok=(n,c,d='')=>{ if(!c){fail++;console.log('  FAIL '+n+(d?'  '+d:''))}else console.log('  ok   '+n); };
// DEMO: hydraulic power P = p*Q
{
  const dom=new JSDOM(readFileSync(B+'/demos/lesson03_motor.html','utf8'),{runScripts:'dangerously',pretendToBeVisual:true});
  const d=dom.window.document,W=dom.window; const $=id=>d.getElementById(id);
  const set=v=>{$('powerRange').value=v;$('powerRange').dispatchEvent(new W.Event('input'));};
  const M=()=>$('motorOut').textContent, E=()=>$('elecOut').textContent, Mg=()=>$('marginOut').textContent;
  set(1.85); ok('1.85 kW demand -> 2.2 kW motor, draws 2.06 kW, 1.19x margin', M()==='2.2' && E()==='2.06' && Mg()==='1.19', M()+'/'+E()+'/'+Mg());
  set(2.30); ok('just over a rung (2.30 kW) jumps to 3.0 kW motor', M()==='3.0', M());
  set(1.00); ok('1.00 kW demand -> 1.1 kW motor (next rung up)', M()==='1.1', M());
  ok('size ladder + demand line + readouts present', ['ladder','bars','demand','demandLab','powerRange','motorOut','marginOut','elecOut','note'].every(id=>$(id)));
}
// QUIZ
{
...
// LESSON
{
  const L=readFileSync(B+'/03_the_electric_motor.md','utf8');
  ...
  ok('DoD6/7 references reviewed SVG in <figure markdown>', L.includes('assets/m05-l3-motor.svg') && /<figure markdown>/.test(L));
  ok('DoD8 demo embedded (iframe) + open-in-tab link', /<iframe[^>]*demos\/lesson03_motor\.html/.test(L) && /\]\(demos\/lesson03_motor\.html\)/.test(L));
  ...
  ok('DoD11 advances narrative (Lesson 02 in, relief/Lesson 04 out)', /Lesson 02|Module 05/.test(L) && /Lesson 04|[Rr]elief/.test(L));
  ok('motor decision present (2.2 kW + next-size margin)', /2\.2\s*kW|motor/i.test(L) && /margin|next.*(size|rung)|standard|1450/i.test(L));
...
console.log(fail?('\n[gate-m5l3] FAIL '+fail):'\n[gate-m5l3] PASS  M05 L03 meets all Definition-of-Done checks + power demo/quiz + crisp vector preview');
