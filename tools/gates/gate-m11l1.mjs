...
const B='/mnt/user-data/outputs/module11_live_model';
let fail=0; const ok=(n,c,d='')=>{ if(!c){fail++;console.log('  FAIL '+n+(d?'  '+d:''))}else console.log('  ok   '+n); };
// DEMO: hydraulic power P = p*Q
{
  const dom=new JSDOM(readFileSync(B+'/demos/lesson01_live_model.html','utf8'),{runScripts:'dangerously',pretendToBeVisual:true});
  const d=dom.window.document,W=dom.window; const $=id=>d.getElementById(id);
  ok('healthy -> residual near noise floor, no fault', parseFloat($('rpk').textContent)<0.3 && $('verdict').className.includes('ok') && $('rstat').textContent==='healthy');
  $('leak').dispatchEvent(new W.Event('click'));     ok('leak -> residual grows far past threshold, flagged', parseFloat($('rpk').textContent)>5 && $('rstat').textContent==='FAULT');
  $('friction').dispatchEvent(new W.Event('click')); ok('friction -> subtler residual, still flagged', $('rstat').textContent==='FAULT' && $('verdict').className.includes('bad'));
  ok('live-model widgets present', ['healthy','leak','friction','curve','rpk','rstat','verdict','thr'].every(id=>$(id)) && $('curve').getAttribute('points').length>0);
}
// QUIZ
{
...
// LESSON
{
  const L=readFileSync(B+'/01_the_live_digital_model.md','utf8');
  ...
  ok('DoD6/7 references reviewed SVG in <figure markdown>', L.includes('assets/m11-l1-live-model.svg') && /<figure markdown>/.test(L));
  ok('DoD8 demo embedded (iframe) + open-in-tab link', /<iframe[^>]*demos\/lesson01_live_model\.html/.test(L) && /\]\(demos\/lesson01_live_model\.html\)/.test(L));
  ...
  ok('DoD11 opens module (Module 09/validated model in, Lesson 02/residual out)', /Module 09|validated model|controller|masks?/i.test(L) && /Lesson 02|residual/i.test(L));
  ok('live-model decision present (compare predicted vs measured)', /live.*model|residual/i.test(L) && /measured.*predicted|predicted.*measured|same command|healthy|fault/i.test(L) && /validated|threshold|behaving as designed/i.test(L));
...
console.log(fail?('\n[gate-m11l1] FAIL '+fail):'\n[gate-m11l1] PASS  M11 L01 meets all Definition-of-Done checks + power demo/quiz + crisp vector preview');
