...
const B='/mnt/user-data/outputs/module10_proportional';
let fail=0; const ok=(n,c,d='')=>{ if(!c){fail++;console.log('  FAIL '+n+(d?'  '+d:''))}else console.log('  ok   '+n); };
// DEMO: hydraulic power P = p*Q
{
  const dom=new JSDOM(readFileSync(B+'/demos/lesson02_proportional.html','utf8'),{runScripts:'dangerously',pretendToBeVisual:true});
  const d=dom.window.document,W=dom.window; const $=id=>d.getElementById(id);
  const setk=v=>{$('kp').value=v;$('kp').dispatchEvent(new W.Event('input'));};
  setk(0.03); ok('low gain -> sluggish, large droop', $('verdict').className.includes('slow') && parseFloat($('rdroop').textContent)>1);
  setk(0.15); ok('Kp=0.15 -> crisp, ~0.08 s, ~0.24 mm droop', $('verdict').className.includes('ok') && /0\.0[0-9] s/.test($('rtau').textContent) && /0\.2[0-9] mm/.test($('rdroop').textContent));
  setk(0.45); ok('high gain -> excites resonance, big overshoot', $('verdict').className.includes('bad') && />200|[4-9][0-9] /.test($('ros').textContent));
  ok('proportional widgets present', ['kp','curve','rtau','rdroop','ros','verdict','tgt'].every(id=>$(id)) && $('curve').getAttribute('points').length>0);
}
// QUIZ
{
...
// LESSON
{
  const L=readFileSync(B+'/02_proportional_control.md','utf8');
  ...
  ok('DoD6/7 references reviewed SVG in <figure markdown>', L.includes('assets/m10-l2-proportional.svg') && /<figure markdown>/.test(L));
  ok('DoD8 demo embedded (iframe) + open-in-tab link', /<iframe[^>]*demos\/lesson02_proportional\.html/.test(L) && /\]\(demos\/lesson02_proportional\.html\)/.test(L));
  ...
  ok('DoD11 advances (Lesson 01 in, Lesson 03/PID out)', /Lesson 01|feedback/i.test(L) && /Lesson 03|PID|integral/i.test(L));
  ok('proportional decision present (gain trade-off, resonance ceiling)', /proportional|K_?p|gain/i.test(L) && /droop|settl|speed/i.test(L) && /resonance|oscillat|12 Hz/i.test(L));
...
console.log(fail?('\n[gate-m10l2] FAIL '+fail):'\n[gate-m10l2] PASS  M10 L02 meets all Definition-of-Done checks + power demo/quiz + crisp vector preview');
