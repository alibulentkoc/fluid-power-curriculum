...
const B='/mnt/user-data/outputs/module10_tuning';
let fail=0; const ok=(n,c,d='')=>{ if(!c){fail++;console.log('  FAIL '+n+(d?'  '+d:''))}else console.log('  ok   '+n); };
// DEMO: hydraulic power P = p*Q
{
  const dom=new JSDOM(readFileSync(B+'/demos/lesson04_tuning.html','utf8'),{runScripts:'dangerously',pretendToBeVisual:true});
  const d=dom.window.document,W=dom.window; const $=id=>d.getElementById(id);
  const setk=(kp,kd)=>{$('kp').value=kp;$('kd').value=kd;$('kp').dispatchEvent(new W.Event('input'));};
  setk(0.15,0.0);  ok('P-only Kp=0.15 -> Ku 0.22, thin margin ~1.5x', /0\.22/.test($('rku').textContent) && /1\.5/.test($('rgm').textContent) && $('verdict').className.includes('thin'));
  setk(0.15,0.01); ok('derivative raises Ku to 0.40 -> healthy 2.7x margin', /0\.40/.test($('rku').textContent) && /2\.[0-9]/.test($('rgm').textContent) && $('verdict').className.includes('ok'));
  setk(0.45,0.01); ok('gain above Ku -> unstable', parseFloat($('rgm').textContent)<1 && $('verdict').className.includes('bad'));
  ok('tuning widgets present', ['kp','kd','rku','rgm','ros','verdict','curve','tgt'].every(id=>$(id)) && $('curve').getAttribute('points').length>0);
}
// QUIZ
{
...
// LESSON
{
  const L=readFileSync(B+'/04_tuning_and_stability.md','utf8');
  ...
  ok('DoD6/7 references reviewed SVG in <figure markdown>', L.includes('assets/m10-l4-tuning.svg') && /<figure markdown>/.test(L));
  ok('DoD8 demo embedded (iframe) + open-in-tab link', /<iframe[^>]*demos\/lesson04_tuning\.html/.test(L) && /\]\(demos\/lesson04_tuning\.html\)/.test(L));
  ...
  ok('DoD11 advances (Lesson 03 in, Lesson 05/mission out)', /Lesson 03|PID|integral|derivative/i.test(L) && /Lesson 05|mission|closed-loop/i.test(L));
  ok('tuning decision present (gain margin vs resonance)', /tun|gain margin|stabilit/i.test(L) && /ultimate gain|K_?u|margin|bandwidth/i.test(L) && /resonance|oscillat|12 Hz/i.test(L));
...
console.log(fail?('\n[gate-m10l4] FAIL '+fail):'\n[gate-m10l4] PASS  M10 L04 meets all Definition-of-Done checks + power demo/quiz + crisp vector preview');
