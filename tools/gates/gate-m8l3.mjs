...
const B='/mnt/user-data/outputs/module08_position';
let fail=0; const ok=(n,c,d='')=>{ if(!c){fail++;console.log('  FAIL '+n+(d?'  '+d:''))}else console.log('  ok   '+n); };
// DEMO: hydraulic power P = p*Q
{
  const dom=new JSDOM(readFileSync(B+'/demos/lesson03_position.html','utf8'),{runScripts:'dangerously',pretendToBeVisual:true});
  const d=dom.window.document,W=dom.window; const $=id=>d.getElementById(id);
  const set=v=>{$('b').value=v;$('b').dispatchEvent(new W.Event('input'));};
  set(12); ok('12-bit -> 0.146 mm, verdict too coarse', /0\.146 mm/.test($('rr').textContent) && $('verdict').className.includes('bad'));
  set(14); ok('14-bit -> 0.037 mm, verdict ok', /0\.0366 mm/.test($('rr').textContent) && $('verdict').className.includes('ok'));
  set(16); ok('16-bit -> ~0.009 mm, verdict ok', /0\.009[0-9] mm/.test($('rr').textContent) && $('verdict').className.includes('ok'));
  ok('sensor-resolution widgets present', ['b','rc','rr','rt','dot','thr','ticks','verdict'].every(id=>$(id)));
}
// QUIZ
{
...
// LESSON
{
  const L=readFileSync(B+'/03_sensing_position.md','utf8');
  ...
  ok('DoD6/7 references reviewed SVG in <figure markdown>', L.includes('assets/m08-l3-position.svg') && /<figure markdown>/.test(L));
  ok('DoD8 demo embedded (iframe) + open-in-tab link', /<iframe[^>]*demos\/lesson03_position\.html/.test(L) && /\]\(demos\/lesson03_position\.html\)/.test(L));
  ...
  ok('DoD11 advances (Lesson 01 in, Lesson 04/pressure out)', /Lesson 0[12]|command|open-loop/i.test(L) && /Lesson 04|pressure|load/i.test(L));
  ok('sensing decision present (resolution vs +/-1 mm)', /resolution|sense|sensor/i.test(L) && /600 mm|14.?bit|0\.1 mm|1 mm/i.test(L));
...
console.log(fail?('\n[gate-m8l3] FAIL '+fail):'\n[gate-m8l3] PASS  M08 L03 meets all Definition-of-Done checks + power demo/quiz + crisp vector preview');
