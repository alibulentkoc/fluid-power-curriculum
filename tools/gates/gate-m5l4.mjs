...
const B='/mnt/user-data/outputs/module05_relief';
let fail=0; const ok=(n,c,d='')=>{ if(!c){fail++;console.log('  FAIL '+n+(d?'  '+d:''))}else console.log('  ok   '+n); };
// DEMO: hydraulic power P = p*Q
{
  const dom=new JSDOM(readFileSync(B+'/demos/lesson04_relief.html','utf8'),{runScripts:'dangerously',pretendToBeVisual:true});
  const d=dom.window.document,W=dom.window; const $=id=>d.getElementById(id);
  const set=v=>{$('reliefRange').value=v;$('reliefRange').dispatchEvent(new W.Event('input'));};
  const H=()=>$('heatOut').textContent, cls=()=>$('status').className;
  set(115); ok('115 bar: safe window, holding dumps 1.92 kW as heat', H()==='1.92' && /\bok\b/.test(cls()), H()+'/'+cls());
  set(102); ok('102 bar: too low -> opens during work (starves platform)', /bad/.test(cls()), cls());
  set(140); ok('140 bar: above component limit -> protects nothing', /bad/.test(cls()), cls());
  ok('setting-window bar + markers + heat readout present', ['bar','safe','workL','limL','relL','reliefRange','heatOut','marginOut','status'].every(id=>$(id)));
}
// QUIZ
{
...
// LESSON
{
  const L=readFileSync(B+'/04_the_relief_valve.md','utf8');
  ...
  ok('DoD6/7 references reviewed SVG in <figure markdown>', L.includes('assets/m05-l4-relief.svg') && /<figure markdown>/.test(L));
  ok('DoD8 demo embedded (iframe) + open-in-tab link', /<iframe[^>]*demos\/lesson04_relief\.html/.test(L) && /\]\(demos\/lesson04_relief\.html\)/.test(L));
  ...
  ok('DoD11 advances narrative (Lesson 03 in, reservoir/Lesson 05 out)', /Lesson 03|Module 05/.test(L) && /Lesson 05|[Rr]eservoir/.test(L));
  ok('relief decision present (~115 bar setting + hold heat)', /115\s*bar|relief/i.test(L) && /window|above.*below|1\.9\s*kW|heat/i.test(L));
...
console.log(fail?('\n[gate-m5l4] FAIL '+fail):'\n[gate-m5l4] PASS  M05 L04 meets all Definition-of-Done checks + power demo/quiz + crisp vector preview');
